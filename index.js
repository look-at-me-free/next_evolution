(() => {
  ...
  const LEFT_RAIL_IDS = [
    "leftRailSlot1", "leftRailSlot2", "leftRailSlot3", "leftRailSlot4",
    "leftRailSlot5", "leftRailSlot6", "leftRailSlot7", "leftRailSlot8",
    "leftRailSlot9", "leftRailSlot10", "leftRailSlot11", "leftRailSlot12"
  ];

  const RIGHT_RAIL_IDS = [
    "rightRailSlot1", "rightRailSlot2", "rightRailSlot3", "rightRailSlot4",
    "rightRailSlot5", "rightRailSlot6", "rightRailSlot7", "rightRailSlot8",
    "rightRailSlot9", "rightRailSlot10", "rightRailSlot11", "rightRailSlot12"
  ];
  ...
  function getSubids(manifest) {
    const fallbackWork = Number(manifest.parent_work_id) || Number(manifest.id) || 1;
  ...
  function wireSearch() {
    ...
    document.addEventListener("click", (e) => {
      if (e.target === input || e.target.closest("#chapterSearchResults")) return;
      results.classList.remove("show");
    });
  ...
  function getEntryContext() {
    ...
    const currentIndex = entries.findIndex(
      entry => normalizeKey(entry.slug) === normalizeKey(CURRENT_ENTRY?.slug)
    );
    ...
    return {
      ...
      next: currentIndex >= 0 && currentIndex < entries.length - 1
  ...
  function buildTraversal(position = "top") {
    ...
    shell.className = "reader-traversal-shell";
    if (position === "bottom") shell.id = "bottomTraversalAnchor";

    const label = document.createElement("p");
    label.className = "reader-traversal-label";
    label.textContent = position === "top" ? "Chapter Navigation" : "Keep The Scroll Alive";
    ...
    if (prev) {
      bar.appendChild(
        makeTraversalPill("← Previous", () => switchEntry(CURRENT_WORK.slug, prev.slug, false))
      );
    }

    for (const entry of entries) {
      const isCurrent = normalizeKey(entry.slug) === normalizeKey(CURRENT_ENTRY?.slug);
      const labelText = entry.subtitle || titleCaseSlug(entry.slug);
      bar.appendChild(
        makeTraversalPill(
          labelText,
    ...
    if (next) {
      bar.appendChild(
        makeTraversalPill("Next →", () => switchEntry(CURRENT_WORK.slug, next.slug, false))
      );
    }
  ...
  function updateChapterProgress(progress = 0) {
    ...
    const pageBar = document.getElementById("chapterProgressBar");
  ...
  function buildChapterMeta(manifest, imageCount) {
    ...
    const workName = manifest.title || CURRENT_WORK.display || titleCaseSlug(CURRENT_WORK.slug);
    const chapterName = manifest.subtitle || CURRENT_ENTRY.subtitle || titleCaseSlug(CURRENT_ENTRY.slug);

    meta.textContent = `${workName} · ${chapterName} · ${imageCount} page${imageCount === 1 ? "" : "s"}`;
    return meta;
  }
  ...
  async function buildReader() {
    ...
    if (heroSubtitle) {
      heroSubtitle.textContent = "At most they simply have to scroll. And that’s easy.";
    }
